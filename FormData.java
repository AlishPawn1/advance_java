...
public class FormData extends JFrame {
    ...
    public FormData() {
        ...
        String[] countries = { "Nepal", "USA", "UK" };
        JComboBox<String> countryBox = new JComboBox<>(countries);
        countryBox.setBounds(250, 300, 150, 30);
        submit = new JButton("Submit");
        submit.setBounds(250, 350, 100, 40);
        add(login); add(username); add(uname); add(userpassword); add(password); add(gender); add(male);
        add(female); add(other); add(hobbies); add(music); add(sports); add(country); add(countryBox);
        add(submit);
        setVisible(true);
        submit.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                String selectedGender = male.isSelected() ? "Male"
                        : female.isSelected() ? "Female" : other.isSelected() ? "Other" : "Not Specified";
                String selectedHobbies = (music.isSelected() ? "Music " : "") +
